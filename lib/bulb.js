  ...
  constructor (platform, config, homebridgeAccessory, bulb) {
    ...
    this.LightbulbService.getCharacteristic(Characteristic.On)
      ...
      .on('set', (value, callback) => {
        this.bulb.lighting.getLightState().then(ls => {
            const defaultState = typeof ls.dft_on_state ==="object" ? ls.dft_on_state : {};
            this.bulb.lighting.setLightState(Object.assign(defaultState, {on_off: value}), { transport: 'udp' }).then(() => {
              callback();
            }).catch((reason) => {
